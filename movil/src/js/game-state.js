...
var leftButtonX = winW - 100;
var leftButtonY = winH - 100;

var rightButtonX = winW - 100;
var rightButtonY = winH - 100;

var gameState = {

    create: function() {
        // tile sprite first so element display on top
        roadSprite = game.add.tileSprite(0, 0, winW, winH, 'road');

        izquierda = game.add.sprite(20,leftButtonY, 'izquierda');
        izquierda.input.start();

        derecha = game.add.sprite(rightButtonX,rightButtonY, 'derecha');
        derecha.input.start();

        cart = game.add.sprite(170, 380, 'cart');
        ...
        particle = game.add.emitter(0, 0, 3);
        ...
        this.labelScore = game.add.text(20, 20, "Puntaje: 0", style);
        this.labelBestScore = game.add.text(20, 40, "Mejor puntaje: " + bestScore, style);
    },

    

    update: function() {
        if (izquierda.input.pointerDown(game.input.activePointer.id)) {
            cart.angle = -15;
            cart.body.acceleration.x = -700;
        }
        else if (derecha.input.pointerDown(game.input.activePointer.id)) {
            cart.angle = 15;
            cart.body.acceleration.x = 700;
        }
        else {
    ...
    addRival: function() {
        ...
        score += 1;
        this.labelScore.content = 'Puntaje: ' + score;
    },
